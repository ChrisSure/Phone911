...
export class TokenInterceptor implements HttpInterceptor {
  ...
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    if (!isLogout && !isRefresh && accessToken != null) {
      ...
      if (timeToRefresh) {
        console.log(`Token interceptor: access token expired on [${request.url}]. Request may fail.`);
        if (this.refreshIsOngoing === false) {
          console.log(`Token interceptor: requesting refresh from [${request.url}].`);
          this.refreshIsOngoing = true;
          this.authService.refresh().subscribe(() => {
            this.refreshIsOngoing = false;
            console.log(`Token interceptor: finishing refresh from [${request.url}].`);
            ...
            console.log(`Token interceptor: refresh from [${request.url}] gave an error. Logging out.`);
            console.log(error);
            this.authService.logout();
          });
        }
        else {
          console.log(`Token interceptor: refresh from [${request.url}] is ignored because it is already in progress.`);
        }
    ...
    return next.handle(request).catch(error => {
      if (!isLogout && error.status === 401) {
        if (this.refreshIsOngoing)
          ...
          this.authService.logout();
          console.log(`Token interceptor: unathorized during [${request.url}], not waiting for refresh. Logging out.`);
        }
